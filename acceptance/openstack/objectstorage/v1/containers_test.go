...
func TestContainers(t *testing.T) {
	...
	for i := 0; i < numContainers; i++ {
		cNames[i] = tools.RandomString("ktvpcsdk-test-container-", 8)
	}
	...
	err = containers.List(client, &containers.ListOpts{Full: true, Prefix: "ktvpcsdk-test-container-"}).EachPage(func(page pagination.Page) (bool, error) {
	...
	err = containers.List(client, &containers.ListOpts{Full: false, Prefix: "ktvpcsdk-test-container-"}).EachPage(func(page pagination.Page) (bool, error) {
...
func TestListAllContainers(t *testing.T) {
	...
	// the 'prefix' parameter is used.
	allPages, err := containers.List(client, &containers.ListOpts{Full: true, Limit: 5, Prefix: "ktvpcsdk-test-container-"}).AllPages()
	th.AssertNoErr(t, err)
	...
	allPages, err = containers.List(client, &containers.ListOpts{Full: false, Limit: 2, Prefix: "ktvpcsdk-test-container-"}).AllPages()
	th.AssertNoErr(t, err)
