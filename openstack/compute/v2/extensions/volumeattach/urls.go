package volumeattach

import "github.com/cloud-barista/ktcloudvpc-sdk-for-drv"

const resourcePath = "os-volume_attachments"

func resourceURL(c *ktvpcsdk.ServiceClient, serverID string) string {
	return c.ServiceURL("servers", serverID, resourcePath)
}

func listURL(c *ktvpcsdk.ServiceClient, serverID string) string {
	return resourceURL(c, serverID)
}

func createURL(c *ktvpcsdk.ServiceClient, serverID string) string {
	return resourceURL(c, serverID)
}

func getURL(c *ktvpcsdk.ServiceClient, serverID, aID string) string {
	return c.ServiceURL("servers", serverID, resourcePath, aID)
}

func deleteURL(c *ktvpcsdk.ServiceClient, serverID, aID string) string {
	return getURL(c, serverID, aID)
}
