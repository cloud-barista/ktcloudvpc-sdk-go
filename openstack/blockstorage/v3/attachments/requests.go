...
func (opts CreateOpts) ToAttachmentCreateMap() (map[string]interface{}, error) {
	return ktvpcsdk.BuildRequestBody(opts, "attachment")
}
...
func Delete(client *ktvpcsdk.ServiceClient, id string) (r DeleteResult) {
	resp, err := client.Delete(deleteURL(client, id), &ktvpcsdk.RequestOpts{
		OkCodes: []int{200},
	})
	_, r.Header, r.Err = ktvpcsdk.ParseResponse(resp, err)
	return
...
func Complete(client *ktvpcsdk.ServiceClient, id string) (r CompleteResult) {
	b := map[string]interface{}{
		"os-complete": nil,
	}
	resp, err := client.Post(completeURL(client, id), b, nil, &ktvpcsdk.RequestOpts{
		OkCodes: []int{204},
	})
	_, r.Header, r.Err = ktvpcsdk.ParseResponse(resp, err)
	return
