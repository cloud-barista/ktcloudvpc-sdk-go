...
func (opts CreateOpts) ToVolumeTypeCreateMap() (map[string]interface{}, error) {
	return ktvpcsdk.BuildRequestBody(opts, "volume_type")
}
...
func ListExtraSpecs(client *ktvpcsdk.ServiceClient, volumeTypeID string) (r ListExtraSpecsResult) {
...
func GetExtraSpec(client *ktvpcsdk.ServiceClient, volumeTypeID string, key string) (r GetExtraSpecResult) {
...
func CreateExtraSpecs(client *ktvpcsdk.ServiceClient, volumeTypeID string, opts CreateExtraSpecsOptsBuilder) (r CreateExtraSpecsResult) {
	b, err := opts.ToVolumeTypeExtraSpecsCreateMap()
	if err != nil {
		r.Err = err
		return
	}
	resp, err := client.Post(extraSpecsCreateURL(client, volumeTypeID), b, &r.Body, &ktvpcsdk.RequestOpts{
		OkCodes: []int{200},
	})
	_, r.Header, r.Err = ktvpcsdk.ParseResponse(resp, err)
	return
...
// for the key in opts.
func UpdateExtraSpec(client *ktvpcsdk.ServiceClient, volumeTypeID string, opts UpdateExtraSpecOptsBuilder) (r UpdateExtraSpecResult) {
	b, key, err := opts.ToVolumeTypeExtraSpecUpdateMap()
	if err != nil {
		r.Err = err
		return
	}
	resp, err := client.Put(extraSpecUpdateURL(client, volumeTypeID, key), b, &r.Body, &ktvpcsdk.RequestOpts{
		OkCodes: []int{200},
	})
	_, r.Header, r.Err = ktvpcsdk.ParseResponse(resp, err)
	return
...
// volume type ID.
func DeleteExtraSpec(client *ktvpcsdk.ServiceClient, volumeTypeID, key string) (r DeleteExtraSpecResult) {
	resp, err := client.Delete(extraSpecDeleteURL(client, volumeTypeID, key), &ktvpcsdk.RequestOpts{
		OkCodes: []int{202},
	})
	_, r.Header, r.Err = ktvpcsdk.ParseResponse(resp, err)
	return
...
func AddAccess(client *ktvpcsdk.ServiceClient, id string, opts AddAccessOptsBuilder) (r AddAccessResult) {
	...
	resp, err := client.Post(accessActionURL(client, id), b, nil, &ktvpcsdk.RequestOpts{
		OkCodes: []int{202},
	})
	_, r.Header, r.Err = ktvpcsdk.ParseResponse(resp, err)
	return
